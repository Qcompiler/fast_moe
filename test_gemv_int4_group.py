...
void warp_specialized_gemv( int32_t* d_A,  half* d_B, half* d_C,
      float* scales, int M, int K, cudaStream_t stream, int groupsize) {
    ...
    if (groupsize == 128)
        sharedMemSize +=  (K * 8 / 128) * sizeof(float) * NUM_WARP;
  

    switch (groupsize) {
      case 128:
        warp_specialized_gemv_kernel_group_128<WARP_PER_ROW, NUM_WARP><<<grid, block, 
        sharedMemSize, stream>>>( d_A, d_B,  d_C, scales,  M, K);
        break;
      case -1:
        warp_specialized_gemv_kernel_group_1<WARP_PER_ROW, NUM_WARP><<<grid, block, 
        sharedMemSize, stream>>>( d_A, d_B,  d_C, scales,  M, K);
        break;
      default:
...
void warp_specialized_gemv_host(cudaStream_t stream, jc::Tensor& weight, 
          ...
          const jc::Tensor& scales, int groupsize) {
  ...
  output.data_ptr<half>(), scales.data_ptr<float>(),  M, K, stream, groupsize);
...
for (out_dim, k) in [ (2048, 2048), (2048, 4096), (4096, 4096), (12288, 4096), (8192, 8192) ]:
  ...
  lib.warp_specialized_gemv_host(q_weight, vector, c, scales, groupsize)
  ...
  with torch.cuda.stream(torch.cuda.Stream()):
    if args.marlin == 1:
      ...
      ms = do_bench(lambda: lib.warp_specialized_gemv_host(q_weight, vector, c, scales, groupsize))
