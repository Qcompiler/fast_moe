...
matmul_type = "GEMV" #GEMM, GEMV, GEMV_REVSPLITK, GEMM_SPLITK | AUTO

#################################################################################################################################
from triton.testing import do_bench
def eval_time(fct, params): 
    return do_bench(lambda: fct(**params), warmup=25, rep=200,   return_mode='min') 
...
def gen_data(in_features, out_features, W_nbits, group_size, device=device):
    ...
    # #################################################################
    # #Marlin
    # from vllm.model_executor.layers.quantization.gptq_marlin import  GPTQMarlinLinearMethod as MarlinLinearMethod
    # from vllm.model_executor.layers.quantization.gptq_marlin import  GPTQMarlinConfig as MarlinConfig

    # if(W_nbits==4):
    #     _marlin_linear = MarlinLinearMethod(MarlinConfig(weight_bits=W_nbits, group_size=group_size, desc_act = False, is_sym = True, lm_head_quantized=False, dynamic=False, full_config = {})) 
...
for batch_size in BATCH_SIZES:
    ...
    if(bitblas_linear is not None):
        ...
        print((batch_size, in_features, out_features), 'Bitblas Speed-up vs. torch.matmul', np.round(ref_time/bitblas_time, 2), 'time', bitblas_time)
