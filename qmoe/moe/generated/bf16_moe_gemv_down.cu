  ...
  if (m < M) {
    ...
    for (int topx = 0; topx < ntopx / each_warp_reduce_compute; topx++ ){
      ...
      for (int i = start + lane; i < end; i += WARP_SIZE) {
          *(((int4 *)shmem_vector) + i) = *(((int4 *)x) + (topx * each_warp_reduce_compute )* (K / 8) + i);
      }
...
__global__ void warp_specialized_gemv_kernel_down(
  ...
  if (m < M) {
    ...
    for (int topx = 0; topx < ntopx; topx++ ){
      ...
      //   __syncthreads();
      for (int i = start + lane; i < end; i += WARP_SIZE) {
          *(((int4 *)shmem_vector) + i) = *(  ((int4 *)x + topx * (K / 8)) + i);
      }
      __syncthreads();
...
void warp_specialized_gemv_down( const half* d_A, const  half* d_B,
    ...
    if (kernel_type == 1)
    {
       ...
       int sharedMemSize = K *  sizeof(half) * each_warp_reduce_compute; // Shared memory for A
        warp_specialized_gemv_kernel_down_split_expert<NUM_WARP, each_warp_reduce_compute><<<grid, block,  sharedMemSize, stream>>>( d_A_, d_B_,  d_C_,  topk_weight, moe_index, ntopx,  M, K);
...
void warp_specialized_gemv_down_host(cudaStream_t stream, const jc::Tensor& down, 
          ...
          const jc::Tensor& topk_weight, const jc::Tensor& topk_ids, int kernel_type) {
