...
def gemv_int4_kernel(
    ...
    for kk in range(0, tl.cdiv(k, BLOCK_SIZE)):
      offs_k =  tl.arange(0, BLOCK_SIZE) + kk * BLOCK_SIZE    
...
def test_dequant_kernel(
    ...
    for kk in range(0, tl.cdiv(int4_k, BLOCK_SIZE)):
      ...
      output_offset = row_id * stride_cm + (offs_k * 8 + 1) * stride_cn
      tl.store(output_ptr + output_offset, a1)

      output_offset = row_id * stride_cm + (offs_k * 8 ) * stride_cn
      tl.store(output_ptr + output_offset, a2)

      output_offset = row_id * stride_cm + (offs_k * 8 + 3) * stride_cn
      tl.store(output_ptr + output_offset, a3)

      output_offset = row_id * stride_cm + (offs_k * 8 + 2) * stride_cn
      tl.store(output_ptr + output_offset, a4)
...
def test_dequant(A: torch.Tensor, vector: torch.Tensor, output, ptx = 0):
    ...
    int4_k = A.shape[1]
    test_dequant_kernel[grid](A, output, n, k, int4_k,  stride_cm, stride_cn, 
                            stride_ak, stride_an,  
...
from common.common import generate_randint, gen_quant4_my, gen_quant4_my_no_reorder
for (out_dim, k) in [ (4096, 4096), (8192, 4096)]:
  dtype = torch.float16
  ...
  #------------------------------------mixq---------------------------------
  c_triton = torch.zeros((out_dim, k), dtype=dtype, device=device)
  test_dequant(q_weight, vector, c_triton)
  print((c_triton - weight).abs().max())
