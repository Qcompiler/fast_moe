      ...
      __syncthreads();
            ...
            #pragma unroll  4
 			 ...
 			 float* scales_ptr =  (float *) (  (int4 *) scales + target_expect * ( ( M * K * 8) / ( 128 * 4 ) ) +  m * ( (K  * 8) / ( 128 * 4 )) ) ;                int k = (w * WARP_SIZE + lane) * NUM_PER_THREAD;
                half2 reg_x_0[4];*(((int4 *)reg_x_0)) =  *(((int4 *)( shmem_vector +  (warp_id % each_warp_reduce_compute) * K)) +  k);
                half2 reg_x_1[4];*(((int4 *)reg_x_1)) =  *(((int4 *)(shmem_vector + (warp_id % each_warp_reduce_compute) * K)) +  k + 1);
